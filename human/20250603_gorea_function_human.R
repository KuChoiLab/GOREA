# ------------------------------------------------------------------
# Script Name: 20250603_gorea_function_human.R
# Author: Hojin Lee
# Date: 2025-06-03
# ------------------------------------------------------------------
...
gorea_enviromnet <- function(directory = NULL) {
  ...
  GOID_TERM <- read.table(paste0(directory, "/GeneOntology/v2024.1.Hs/GOBP/20250124_c5.go.bp.v2024.1.Hs.term.id_mapping.txt"),
                          header = T,
                          sep = "\t") # this file is made from json file
  GO.ANCESTOR <<- readRDS(paste0(directory, "/GeneOntology/v2024.1.Hs/GOBP/20250124_c5.go.bp.v2024.1.Hs_ANCESTOR_df.rds"))
  GO.LEVEL <<- readRDS(paste0(directory, "/GeneOntology/v2024.1.Hs/GOBP/20250124_c5.go.bp.v2024.1.Hs_LEVEL_df.rds"))
  ...
  GO.LARGE <<- readRDS(paste0(directory, "/GeneOntology/v2024.1.Hs/GOBP/20250206_msigDB_GO_ancestor_large.rds"))
...
gorea_clustering <- function(cutoff = 0.84,
  ...
  if (outlier_detect == T) {
    ...
    outlier_clusters <- bc_res %>% dplyr::group_by(cluster) %>% dplyr::summarise(n = n(), .groups = "drop") %>% dplyr::filter(n < min_cluster) %>% dplyr::pull(cluster)
...
gorea_getorder <- function(w = NULL,
  ...
  if (outlier_detect == T) {
    ...
    score_sort <- final_order %>% dplyr::group_by(cluster) %>% dplyr::summarise(score_mean = mean(!!sym(score)), .groups = "drop") %>% dplyr::arrange(desc(score_mean))
...
gorea_getHeatmap <- function(k_val = NULL,
  ...
  if (top_ancestor_annotation == T) {
    ...
    res <- GO.ANCESTOR.LEVEL %>% dplyr::filter(TARGET_GOID %in% gorea_summary_res$SummaryGO_1$GOID) %>%
      ...
      dplyr::group_by(ANCESTOR_GOTERM, ANCESTOR_LEVEL, ANCESTOR_GOID) %>% dplyr::summarise(n=n(), .groups = "drop") %>% dplyr::arrange(desc(n))
    ...
    set.seed(1234); hc <- stats::hclust(dmat, method = "ward.D") # h.clust using ward.D.
    set.seed(1234); large_hclust_wardD <- stats::cutree(hc, k = top_ancestor_annotation_number)
    ...
    for (i in 1:top_ancestor_annotation_number) { ht1@anno_list[[names(ht1@anno_list)[i]]]@color_mapping@levels <- c("child term", " ")}
  ...
  total_clusters <- final_order %>% dplyr::group_by(cluster) %>% dplyr::summarise(n=n(), .groups = "drop") %>% dplyr::select(cluster) %>% table() %>% names()
...
gorea <- function(input = input_df, k_val = NULL, godata_GO = NULL,
  ...
  gorea_getHeatmap_res <- gorea_getHeatmap(k_val = k_val,
                                           ...
                                           top_ancestor_annotation_number = top_ancestor_annotation_number,
                                           color = color)
